...
const dom = {
    ...
    reloadDatabase: getElementByIdOrThrow("reload-database"),
    syncButton: getElementByIdOrThrow("sync-live-data"),
    symbolInput: getElementByIdOrThrow("symbol-input"),
    tickerSelect: getElementByIdOrThrow("ticker-select"),
    providerStatus: getElementByIdOrThrow("provider-status"),
    ingestStatus: getElementByIdOrThrow("ingest-status"),
    ...
    visualizationMount: getElementByIdOrThrow("visualization-mount"),
    visualizationMeta: getElementByIdOrThrow("visualization-meta"),
};
...
async function loadBootstrap() {
    ...
    const payload = await loadJson(`/api/bootstrap${query ? `?${query}` : ""}`);
...
async function syncLiveData() {
    ...
    try {
        const payload = await loadJson("/api/ingest", {
            method: "POST",
            headers: {
...
function buildSummary(signals, events, provider) {
...
class MarketScene {
    constructor(mount) {
        ...
        const floor = new THREE.Mesh(new THREE.CircleGeometry(10, 64), new THREE.MeshStandardMaterial({ color: 0xf0dfca, transparent: true, opacity: 0.45 }));
